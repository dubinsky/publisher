...
=== Transclusion
Sort the pages in transclusion order, transclude, and style the transclusions
...
- include (some of the) SyntheticPages in the link resolution
or introduce (non-transcludable) Link.ToAnchor?

=== Markdown
- catch parsing exceptions and convert them
- add FlexMark footnotes extension (Obsidian does not seem to support footnotes) - and deal with the "Unknown entity reference: &#8617;"
- add FlexMark gfm.tasklist extension - and deal with the resulting error from ZIO Blocks XML parser (I guess): "Unknown entity reference: &nbsp;"; time to change parsers!
- add GitHub Flavored Markup to FlexMark? Probably no, but loot at https://github.com/KaTeX/KaTeX[KaTeX] as a MathJax alternative...
...
=== Sections Hierarchy
- nest HTML-like sections while gathering them and verify the nesting levels
- code retrieval of a section for transclusion
- code section gathering for XML-like formats (TEI DocBook)
- remove HTML-specific `level` from the general `Page.Section`

=== Back-links
- shorten the context to 120-130 characters
- make mentions links to the source of the link; for that:
- add ids to all (internal) links
...
- unify creation of HTML lists, references etc.
- display page icon in the list
- escape URLs etc.?
