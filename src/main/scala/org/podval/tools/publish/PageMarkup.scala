...
final class PageMarkup(
  ...
  private lazy val blocks: Seq[Block] =
    if !markup.recognizeBlocks then Seq.empty else markup.gather(xml, element =>
  ...
  private def process(xml: Xml.Element): Xml.Element = Xml.transform(
    xml,
    stop = element => markup.stop(Xml.qName(element)),
    element =>
  ...
  private def resolveLinks(element: Xml.Element): Xml.Element = Xml.transform(
    element,
    stop = element => markup.stop(Xml.qName(element)),
    element =>
      val transformed: Option[Xml.Element] = internalLinkRef(element).map: ref =>
        site.resolveRef(ref) match
          case None =>
